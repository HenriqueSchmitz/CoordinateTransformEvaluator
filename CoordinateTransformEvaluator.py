...
def addPoint(event, x, y, flags, param):
  if event == cv2.EVENT_LBUTTONUP:
    cv2.circle(rescaled, (x, y), 3, (0, 255, 0), -1)
    fullImage = differentSizedVerticalConcat([rescaled, rescaledReferenceMap])
    cv2.imshow("Example Image", fullImage)
    cv2.waitKey(1)
    print(param)
...
calibrator = TransformCalibrator(rescaledReferenceMap, rescaled)
...
transform = calibrator.run(mapPoints,["1", "2", "3", "4"], ["1", "2", "5"])
...
imageWithDetections = detector.drawDetectionsOnImage(testedImage, detections, ["robot"])
